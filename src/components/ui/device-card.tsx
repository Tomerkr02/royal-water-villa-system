...
export function DeviceCard({
  ...
  return (
    <motion.button
      ...
      onClick={onToggle}
      className={cn(
        "relative overflow-hidden rounded-[1.7rem] border p-4 transition-all duration-300 lg:rounded-[1.8rem]",
      ...
      <div className="relative flex h-full flex-col justify-between gap-4">
        <div className={cn("flex items-start justify-between gap-3", dir === "rtl" ? "flex-row-reverse" : "flex-row")}>
          ...
          <div className="min-w-0 flex-1">
            ...
            <h3 className="mt-2 text-[1.2rem] font-semibold leading-6 text-foreground">{copy.name}</h3>
            <p className="mt-1.5 max-w-xs text-xs leading-5 text-white/58">{copy.description}</p>
          </div>
        </div>
        ...
        <div className={cn("flex items-end justify-between gap-3", dir === "rtl" ? "flex-row-reverse" : "flex-row")}>
          <span
            className={cn(
              "inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-[11px] tracking-[0.18em]",
              device.isOn
          ...
          <div className={dir === "rtl" ? "text-right" : "text-left"}>
            ...
            <p className={cn("mt-1.5 text-base font-semibold", device.isOn ? "text-emerald-100" : "text-white/55")}>
